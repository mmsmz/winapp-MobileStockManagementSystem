...
{
    public partial class transfer_colombo : Form
    {
        SqlConnection conn;
        private int indexRow;

        public transfer_colombo()
        {
            try
            {
        ...
        private void transfer_colombo_Load(object sender, EventArgs e)
        {
            try
            {
                conn.Close();
        ...
        private void DisplayData()
        {
            ...
            SqlDataAdapter adapt = new SqlDataAdapter("select * from colombo_branch", conn);
            adapt.Fill(dt);
        ...
        private void btnkandy_Click(object sender, EventArgs e)
        {
            ...
            if (!(txtitemno.Text.Equals("")))
            {
                try
                {
                    conn.Close();
                    conn.Open();
                    string search = "SELECT * FROM colombo_branch WHERE Item_No = '" + txtitemno.Text + "'";
                    ...
                    if (!(txtitemno.Text.Equals("")))
                    {
                        ...
                        conn.Open();
                        SqlCommand deleteCommand = new SqlCommand("delete from colombo_branch where Item_No=@Item_No", conn);
        ...
        private void btngalle_Click(object sender, EventArgs e)
        {
            if (!(txtitemno.Text.Equals("")))
            {
                try
                {
                    ...
                    {
                        conn.Open();
                        string insert = "INSERT INTO Galle_branch VALUES('" + ItemNo + "', '" + BrandName + "','" + ModelName + "', '" + Version + "', '" + LaunchedDate + "', '" + Price + "')";
                        SqlDataAdapter func = new SqlDataAdapter(insert, conn);
                        func.SelectCommand.ExecuteNonQuery();
                        conn.Close();
                        MessageBox.Show("You have transfered the data to Galle Branch Successfully ", "Successfully Transfered", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void textBox1_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            cmd.CommandText = "select * from Colombo_branch where Item_No like ('" + textBox1.Text + "%')";
            cmd.ExecuteNonQuery();
            DataTable dt = new DataTable();
